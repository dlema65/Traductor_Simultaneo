...
from watson_helper import SttWrapper, TtsWrapper, TranslatorWrapper
...
def listen():
    ...
    if recorder.is_recording:  # está encendida?
        ...
        # borra el contenido de la caja de texto escuchado y traducido
        ...
        # traduce
        translation = translator.translate(listened_phrase, origin_language.get(), target_language.get())

        # escribe texto en la caja de texto escuchado
        listened_text.insert('1.0', listened_phrase)
        translated_text.insert('1.0', translation.result['translations'][0]['translation'])
...
def read_aloud():
    """
    Reacción al botón de leer (speak)
    """
    # verifica que el texto a leer no esté vacío
    if len(translated_text.get('1.0', 'end-1c')) > 0:
        # lee
        audio = tts.tts(translated_text.get('1.0', 'end-1c'), target_language.get())
        recorder.play(audio.result.content)
...
read_button = tk.Button(translation_frame,
                        ...
                        bg=SPRING_GREEN,
                        activebackground=rgba_to_rgb(SPRING_GREEN + 'CC'),
                        command=read_aloud,   # agregada función para leer
                        )
